...
public class PlayerMoveListener implements Listener {
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		if (event.getFrom().getBlock().getBiome() != event.getTo().getBlock().getBiome()) {
			...
			// check for completely disabled biomes and biomes that don't count towards the
			// achievement.
			// it would be nice to announce the ones that are new but non-achievement
			// biomes, but we have no way
			// of tracking them without creating out own additional file. the problem with
			// this is that the minecraft
			// achievement changes all the time. at the time of this writing, they just
			// added 4 more biomes to it.
			// this would be a mess to maintain - so we only count achievement biomes.
			if (BiomeLists.isNonAchievementBiome(newBiome)) {
				return;
			}
			if (!biomeList.contains(newBiome)) {
				biomeList.add(newBiome);
				String message = ChatColor.GREEN + "You've found a " + ChatColor.AQUA
						+ BiomeFormatter.formatBiome(newBiome) + ChatColor.GREEN + " biome!";
