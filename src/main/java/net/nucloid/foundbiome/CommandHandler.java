...
public class CommandHandler implements CommandExecutor {
	...
	private void getUnseenBiomes(CommandSender sender) {
		...
		for (Biome b : Biome.values()) {
			/*
			 * A lot of biomes are disabled here. We don't show them because they don't
			 * count towards the achievement and cannot be tracked without creating extra
			 * files.
			 */
			if (!playerBiomes.contains(b) && !b.name().startsWith("MUTATED") && !BiomeLists.isNonAchievementBiome(b)) {
	...
	/**
	 * For the sake of readability (there are quite a few biomes to print out), this
	 * method takes a number. As long as the number increments, it will return a
	 * different color every time it is called, alternating between Aqua and Dark
	 * Aqua. It will append the colored biome to the provided StringBuilder object.
	 * 
	 * @param sb    The StringBuilder to append to.
	 * @param b     The Biome whose name will be appended.
	 * @param color A number, which if even will return a dark aqua string. If odd,
	 *              aqua.
	 */
